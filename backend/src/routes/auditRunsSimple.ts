...
auditRunsRouter.post('/technical-analyzer/run', async (req: Request, res: Response) => {
  try {
    ...
    if (db) {
      ...
      let site: { id: string; domain: string } | null = null;
      ...
      try {
        ...
        if (expectedProjectId) {
          const siteRes = await tx.query<{ id: string; domain: string }>(
            'SELECT id, domain FROM sites WHERE id = $1 FOR UPDATE',
            [expectedProjectId],
          );
        ...
        (async () => {
          try {
            ...
            for (const seed of submittedSeeds) {
              try {
                const result = await auditSingleUrl(seed.url, seenTitles, seed.pageType);
